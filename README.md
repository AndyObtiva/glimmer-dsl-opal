...
- **Images Are Local** Desktop apps typically display local images included in the app files. When running a desktop app on the web, Glimmer DSL for Opal automatically copies application images to assets directory and exposes them as download links. Ruby code that relies on File.expand_path or File.join automatically detects available web URLs for images and matches the web URL to the desktop image path behind the scenes, so desktop app images show up on the web without any extra effort! 
