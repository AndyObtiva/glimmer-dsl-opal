...
#         observe(model, model_binding.property_name_expression)
      ...
      def populate_table(model_collection, parent, column_properties)
        selected_table_item_models = parent.selection.map(&:data)
        parent.remove_all
        model_collection.each do |model|
          table_item = TableItem.new(parent)
          for index in 0..(column_properties.size-1)
            table_item.text = index, model.send(column_properties[index]).to_s
          end
          table_item.data = model
        end
        selected_table_items = parent.search {|item| selected_table_item_models.include?(item.data) }
